...
def set_logging_config(log_level: str, log_file: str = None, only_logs: bool = False):
    ...
    stream_formatter = logging.Formatter(
        "\n%(asctime)s [File: %(filename)s:%(lineno)d] \t"
        "[Module: %(module)s]\t %(levelname)s: %(message)s"
    )
    log_file_formatter = logging.Formatter(
        '{"timestamp": "%(asctime)s", "filename": "%(filename)s:%(lineno)d", '
        '"level": "%(levelname)s", "module": "%(module)s", "message": "%(message)s"}'
    )
